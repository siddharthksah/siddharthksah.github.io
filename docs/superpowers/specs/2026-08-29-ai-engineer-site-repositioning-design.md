...
5. **Selected publications** block — 3–4 hand-picked papers with venue and year, linking to the papers directly. Confirmed-real seed entries: "Freeform liquid 3D printing of soft functional components for soft robotics" (ACS Applied Materials & Interfaces, 2022) and "A miniaturized optical tomography platform for volumetric imaging of engineered living systems" (Lab on a Chip, 2019 — note: no "and portable" in the real title; an earlier draft got this wrong). Owner confirms any additions (candidates needing verification: the IPTC 2026 worker-safety AI paper, TENCON 2024 cyber-threat knowledge graph, ThiefGuard 2025).
