...
About twenty-three containers run the usual self-hosting roster plus my own work: [Immich](https://immich.app/) for photos, [paperless-ngx](https://github.com/paperless-ngx/paperless-ngx) for documents, [Home Assistant](https://www.home-assistant.io/), audiobooks in [Audiobookshelf](https://www.audiobookshelf.org/), PDF tooling from [Stirling PDF](https://github.com/Stirling-Tools/Stirling-PDF), [Glance](https://github.com/glanceapp/glance) dashboards, a local LLM front end in [Open WebUI](https://github.com/open-webui/open-webui), and [SnapOtter](https://snapotter.com/), which I develop and dogfood here. [Nginx Proxy Manager](https://nginxproxymanager.com/) terminates TLS, [Pi-hole](https://pi-hole.net/) on the Raspberry Pi filters DNS, [Tailscale](https://tailscale.com/) carries everything administrative, and the router forwards exactly two ports.

An external request does not go straight to Docker, and the reason is the most macOS-specific fact in this post. [Docker Desktop](https://www.docker.com/products/docker-desktop/) runs containers inside a VM and [NATs](https://en.wikipedia.org/wiki/Network_address_translation) every forwarded connection, so by the time a request reaches a containerized proxy, every client on earth appears to come from the same gateway address. Ban tooling that reads those logs learns nothing. So a native nginx on the host owns ports 80 and 443, logs real client addresses, and forwards into Docker, where NPM does TLS and routing. Two proxies deep before a service sees a byte, and each layer exists for a reason the other cannot cover.
...
Secrets and the small databases are encrypted with [age](https://github.com/FiloSottile/age) before they leave the machine and pushed to a private git vault with a trick I like: each push is an orphan commit force-pushed over the last, so the repo carries tonight's backup and no history, and stays small forever. Oversized artifacts get slimmed to fit, including the uptime monitor's database, which drops from hundreds of megabytes to a few once the heartbeat history tables are purged. The age key lives in the password manager, which is the recovery path if the laptop itself is gone. Bulk media rides a separate cloud backup.
...
The habit started in 2016 with an [Arduino](https://en.wikipedia.org/wiki/Arduino), an RF remote, and a motorized lock on a dorm room door. The hardware got better and the failure modes got politer. The itch is the same.
