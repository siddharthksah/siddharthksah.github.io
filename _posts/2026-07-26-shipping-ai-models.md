...
The AI scripts run in a long-lived [Python dispatcher](https://github.com/snapotter-hq/SnapOtter/blob/main/packages/ai/python/dispatcher.py) that keeps heavy imports warm, gates scripts through a hardcoded allowlist, and cleans up GPU memory after every job. All those scripts share one virtual environment, and that sharing causes a problem.
...
Session creation is the other cost people don't budget for. Building an [ONNX Runtime](https://onnxruntime.ai/) session optimizes the model graph and sets up the backend, which takes hundreds of milliseconds to seconds on a small box and allocates memory that persists. Animated GIFs showed why sessions have to be reused. Instantiating a fresh background-removal session per frame thrashed memory on small boxes, so one session now serves every frame of the animation.
